...
public class Delaunay {
	...
	private void changeFlatTriangle(MyTriangle aTriangle,
		...
		for (int i = 0; i < nbElements; i++) {
			...
			int iter = 20;
			while ((Math.abs(middle.z - basicZ) < MyTools.epsilon)
					&& (iter > 0)) {
	...
	private void removeTriangle(MyTriangle aTriangle, MyEdge removeEdge) {
		...
		int type = removeEdge.getType();
		int topo = removeEdge.getTopo();
		...
		if (type != 0) {
			removeEdge.setType(0);

			for (int i = 0; i < 2; i++) {
				if (remain[k].getType() == 0)
					remain[k].setType(type);
			}
		}

		if (topo != 0) {
			removeEdge.setTopo(0);

			for (int i = 0; i < 2; i++) {
				if (remain[k].getTopo() == 0)
					remain[k].setTopo(type);
			}
		}
	}
